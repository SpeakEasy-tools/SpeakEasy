...
function DataValidator({ rawFiles, setParsedFiles, removeFile }) {
        ...
        if (/(\.csv|\.tsv)$/i.exec(file.name)) {
            ...
            reader.onload = function(evt) {
                ...
                workBook.SheetNames.forEach(s => {
                    const j = JSON.parse(
                        JSON.stringify(
                            XLSX.utils.sheet_to_json(workBook.Sheets[s])
                        )
                    );
            ...
            reader.onload = function(evt) {
                setParses(prevState => [
                    ...prevState,
                    { ...JSON.parse(evt.target.result), fileName: file.name }
                ]);
            };
            reader.readAsText(file);
        }
    ...
    useEffect(() => {
        ...
        if (
            parses.every(p => p.x === parses[0].x) &&
            typeof parses[0].data[0][parses[0].x] === "number"
        ) {
        ...
        <div className={clsx(classes.root)}>
            <div className={clsx(classes.row)}>
                <div className={clsx(classes.pad)}>
                    ...
                    >
                        Use the check boxes to confirm your axes and series
                        labels.
                    </Typography>
                </div>
                {xDisabled && (
...
DataValidator.displayName = "DataValidator";
DataValidator.propTypes = {
    rawFiles: PropTypes.any,
    setParsedFiles: PropTypes.any,
