...
export const InsertVocabularyLists = (
    lessonId,
    userId,
    name,
    description,
     ...
     */
    const [insertVocabularyLists, { data }] = useMutation(
        INSERT_VOCABULARY_LISTS
    );
    ...
    useEffect(() => {
        if (!(lessonId && userId && isPublic != null && name && description))
            return;
...
export const UpdateVocabularyList = (vocabularyListId, name, description) => {
    ...
    const UPDATE_VOCABULARY_LISTS = gql`
        mutation updateVocabularyLists(
            $vocabularyListId: Int!
            $name: String!
            $description: String!
        ) {
            update_vocabulary_lists(
                where: { id: { _eq: $vocabularyListId } }
                _set: {
                    description: $description
                    name: $name
     ...
     * number of configurations returned.
     */
    const [updateVocabularyLists, { data }] = useMutation(
        UPDATE_VOCABULARY_LISTS
    );
...
export const DeleteVocabularyList = vocabularyListId => {
     ...
     * number of configurations returned.
     */
    const [deleteVocabularyLists, { data }] = useMutation(
        DELETE_VOCABULARY_LISTS
    );
    const [rowsAffected, setRowsAffected] = useState(null);
