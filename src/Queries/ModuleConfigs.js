...
export const GetConfigs = moduleId => {
    ...
    const MODULE_CONFIG = gql`
        query getConfig($moduleId: Int!) {
            module_configs(where: { module: { id: { _eq: $moduleId } } }) {
                id
                name
                config
            }
...
export const GetConfigsByName = moduleName => {
    ...
    const MODULE_CONFIG = gql`
        query getConfig($moduleName: String!) {
            module_configs(where: { module: { name: { _eq: $moduleName } } }) {
                id
                name
                config
            }
...
export const InsertConfig = (moduleId, name, config) => {
    ...
    const INSERT_CONFIG = gql`
        mutation insertConfig(
            $moduleId: Int!
            $name: String!
            $config: jsonb!
        ) {
            insert_module_configs(
                objects: { module_id: $moduleId, name: $name, config: $config }
            ) {
                affected_rows
            }
        }
    ...
    useEffect(() => {
        if (
            !(
                data &&
                data["insert_module_configs"] &&
                data["insert_module_configs"]["affected_rows"] > 0
            )
        )
            return;
    ...
    const UPDATE_CONFIGS = gql`
        mutation updateConfig(
            ...
            update_module_configs(
                where: { id: { _eq: $configId } }
                _set: { config: $config, name: $name }
            ) {
                affected_rows
            }
        }
